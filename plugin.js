    ...
    if (!global.XenoLib || !global.ZeresPluginLibrary || global.DiscordJS || XenoLibOutdated || ZeresPluginLibraryOutdated) {
      ...
      class k extends BdApi.React.PureComponent {
        constructor(a) {
          super(a), this.state = {
            hasError: !1
          }, this.componentDidCatch = a => (console.error(`Error in ${this.props.label}, screenshot or copy paste the error above to RexTheDev for help.`), this.setState({
            hasError: !0
  ...
  openWindow(type) {
    ...
    const loadMessages = () => {
      ...
      try {
        ...
        if (loadAttempts > 4) {
          XenoLib.Notifications.error(`Couldn't load menu messages! Report this issue to RexTheDev, error info is in console`, { timeout: 0 });
